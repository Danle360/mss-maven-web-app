...
pipeline {
  ...
  stages {
    ...
    stage ('Dev Environment Deployment') {
      ...
      deploy adapters: [tomcat9(credentialsId: 'apache-tomcatt11-username-passord', path: '', url: 'http://3.17.180.247:8085/')], contextPath: 'mss-walmart-dev-app', war: '**/*.war'
      }
    }
    ...
    stage ('QA Deploy, Application functional Testing Has Started') {
      ...
      deploy adapters: [tomcat9(credentialsId: 'apache-tomcatt11-username-passord', path: '', url: 'http://3.17.180.247:8085/')], contextPath: 'mss-walmart-qa-app', war: '**/*.war'
      }
    }
